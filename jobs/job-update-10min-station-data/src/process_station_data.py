...
def process_live_weather_data():
    ...
    if not (bucket_name and region and endpoint_url):
        print(
            "Error: Required environment variables not set (BUCKET_NAME, REGION, ENDPOINT_URL)"
        )
        sys.exit(1)
...
def process_hyras_rolling_avg_file(station_id, output_id, rolling_avg_file):
    ...
    print(
        f"Processing HYRAS rolling average data for station {station_id} (output_id: {output_id})"
    )
...
def process_interpolated_hourly_temperatures(station_data_file, time_periods=None):
    ...
    print(
        f"Processing interpolated hourly temperatures for {len(time_periods)} time periods"
    )
    ...
    # Pay attention to the date format in the station data, because that can change sometimes, based on DWD developers.
    # In the past, the format changed multiple times without notice.
    station_data["data_date"] = pd.to_datetime(
        station_data["data_date"], format="%d.%m.%Y %H:%M"
    )
    ...
    for (month, day), group_data in date_groups:
        ...
        for period in time_periods:
            ...
            s3_path = (
                f"data/interpolated_hourly/{period_start}_{period_end}/{file_name}"
            )
            ...
            try:
                ...
                print(
                    f"Downloaded and loaded interpolated temperatures for {period} on {month_str}-{day_str}"
                )
            except Exception as e:
                print(
                    f"Warning: Failed to download interpolated temperatures for {period} on {month_str}-{day_str}: {str(e)}"
                )
        ...
        for idx, row in group_data.iterrows():
            ...
            for period in time_periods:
                ...
                if (
                    interpolated_files[period] is not None
                    and station_id
                    in interpolated_files[period]["station_id"].astype(str).values
                ):
                    # Find the row with matching station_id
                    station_row = interpolated_files[period][
                        interpolated_files[period]["station_id"].astype(str)
                        == station_id
                    ]
    ...
    try:
        upload_file(
            updated_file, bucket_name, region, endpoint_url, directory="station_data"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process station data and return output file path"
    )
    parser.add_argument(
        "--return-output-path",
        action="store_true",
